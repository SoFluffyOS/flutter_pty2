...
Future<void> main() async {
  ...
  for (final size in benchmarkSizes) {
    final expected = benchmarkPattern(size);
    final summary = await runBenchmark('output', () async {
      final session = await Pty.spawn(
        benchmarkFixtureOptions(['flood-output', '$size']),
      );
      try {
        ...
        _expectSuccessfulTransfer(exit, output, expected);
...
void _expectSuccessfulTransfer(
  ...
  if (exit case PtyExitCode(:final code) when code != 0) {
    throw StateError('Output benchmark child exited with code $code.');
  }
  if (output.length != expected.length) {
    throw StateError(
      'Output benchmark received ${output.length} bytes, '
      'expected ${expected.length}.',
    );
  }
  for (var index = 0; index < output.length; index++) {
    if (output[index] != expected[index]) {
      throw StateError('Output benchmark mismatch at byte $index.');
    }
  }
}
